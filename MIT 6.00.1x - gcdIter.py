# -*- coding: utf-8 -*-
"""
Created on Sun Feb 18 13:29:52 2018

@author: Zeljko
"""


#Exercise: gcd iter

"""
The greatest common divisor of two positive integers is 
the largest integer that divides each of them without remainder. For example, 

    gcd(2, 12) = 2

    gcd(6, 12) = 6

    gcd(9, 12) = 3

    gcd(17, 12) = 1

Write an iterative function, gcdIter(a, b), that implements 
this idea. One easy way to do this is to begin with a 
test value equal to the smaller of the two input arguments, 
and iteratively reduce this test value by 1 until you either 
reach a case where the test divides both a and b without 
remainder, or you reach 1. 





"""

def gcdIter(a, b):
    '''
    a, b: positive integers
    
    returns: a positive integer, the greatest common divisor of a & b.
    '''
    guess = 0
    if a > b:
        
        guess = b
        for i in range(guess, 0, -1):
            if a % guess == 0 and b % guess == 0:
                return guess
                break
            else:
                guess -= 1
    else:
        guess = a
        for i in range(guess, 0, -1):
            if a % guess == 0 and b % guess == 0:
                return guess
                break
            else:
                guess -= 1
            

print("Result is: ", gcdIter(1071, 462))


#%%

#solution with the recursion instead of iteration.

def gcdRecur(a, b):
    '''
    a, b: positive integers
    
    returns: a positive integer, the greatest common divisor of a & b.
    '''
    
    if b == 0:
        return a
    else:
        return gcdRecur(b, a % b)
    
print("Result is: ", gcdRecur(9,12))

#%%